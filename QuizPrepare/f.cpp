...
int get_depth(node * current) {
    if (current == NULL) return 0;
    int left_son = get_depth(current->left) + 1;
    int right_son = get_depth(current->right) + 1;
    return max(left_son, right_son);
}

int get_leaves(node * root) {
    if (root == NULL) return 0;
    if (root->left == NULL && root->right == NULL) {
        return 1;
    }
    int left_son = get_leaves(root->left);
    int right_son = get_leaves(root->right);
...
void write()
{
    ...
    int leaves_cnt = get_leaves(root);
    cout << leaves_cnt << endl;
}
